...
-- osm_all_point
create view osm.osm_all_point as (
  select
    "osm_id",
...
-- osm_all_line
create view osm.osm_all_line as (
  select
    "osm_id",
...
-- osm_all_polygon
create view osm.osm_all_polygon as (
  select
    "osm_id",
    (CASE
...
-- osm_all_rel
create view osm.osm_all_rel as (
  select
    "osm_id",
...
-- osm_poipoly
create view osm.osm_poipoly as (
  select * from osm_all_point
  union all
  select * from osm_all_polygon
);
...
-- osm_linepoly
create view osm.osm_linepoly as (
  select * from osm_all_line
  union all
  select * from osm_all_polygon
);
...
create view osm.osm_rel_members as (
  select
    osm_rel.osm_id,
