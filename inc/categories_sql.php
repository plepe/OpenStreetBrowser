...
//  else
  ...
  $from="from osm_$table(!bbox!, $\${$where}$$)";

  $select=implode(", ", $select);
  return "select t2.id as id, t2.geo, t2.tags as tags, t2.result->'rule_id' as rule_id, t2.result->'importance' as importance, result as rule_tags from (select {$select} {$from}) as t2 where t2.result->'importance'='$importance'";// group by t2.result[1], t2.result[2], t2.result[3]";
